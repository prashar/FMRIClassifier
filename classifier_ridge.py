...
class FMRIWordClassifier:
    ...
    def SplitData(self,split,total_rows,col):
        ...
        while( i <= total_rows ):
          if((i%25) == 0 and  i > 20):
            a_cop[a_idx:a_idx+20] = a[i-25:i-5]
            print 'copy a from ' , i-25, ' ',  i - 5
            a_idx += 20
            b_cop[b_idx:b_idx+5] = a[i-5:i]
            print 'copy b from ' , i-5, ' ',  i
            b_idx += 5
          i += 1
        print a_idx,b_idx
        #print wordid_train
        ...
        a_vec_w = a_cop
        b_vec_w = b_cop
        self.fmri_train = a_vec_w
        self.fmri_test = b_vec_w
        #mmwrite('sfmri_train_240.out',a_vec_w,field='real',precision=25)
        #mmwrite('sfmri_train_60.out',b_vec_w,field='real',precision=25)
        #mmwrite('wordid_train_240.out',a_vec_w,field='integer')
        #mmwrite('wordid_train_60.out',b_vec_w,field='integer')


    def setup_for_ridge(self,lamb,comps):
        ...
        # In order to append the 218xcomps vector
        final_weight_vector = zeros((218,comps+1))
        for i in range(0,218):
            t1 = time()
            w_init = zeros(comps+1)
            weight_vector_i = self.solve_ridge(lamb,self.fmri_train,Y,i,w_init)
        ...
        io.mmio.mmwrite('pca_weight_vec{0}.out'.format(lamb),sparse_weight_vector,field='real',precision=25)
    ...
    def read_data(self):
        ...
        # 300 X # of components
        self.fmri_all = io.mmio.mmread("PCAProcessedTrainingSet/fmri_train_300samples_300components_randomizedpca.out.mtx")
        self.SplitData(240,300,300)
        #self.fmri_train = self.fmri_all[:240,:]
        # 60 X # of components
        #self.fmri_test = io.mmio.mmread("fmri/subject1_fmri_std.test.mtx")
        #self.fmri_test = self.fmri_all[240:,:]
        ...
        self.wordid_test = io.mmio.mmread("wordid_60_2cand.out.mtx")
